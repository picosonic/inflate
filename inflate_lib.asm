...
; :24  EQUB 7
; :6   EQUB 8
; :2   EQUB 8 ; meaningless codes
; :30  EQUB 5+DISTANCE_TREE
	lda	#5+DISTANCE_TREE
	cpy	#LENGTH_SYMBOLS
	bcs	inflateCompressed_setControlCodeLength
	cpy	#24
	adc	#256-(DISTANCE_TREE-2); #2-DISTANCE_TREE (f2)
.inflateCompressed_setControlCodeLength
	cpy	#CONTROL_SYMBOLS
	bcs skip1:sta	controlSymbolCodeLength,y
.skip1
	iny
...
;	lda	#0
	ldx	#256-(4*TREE_SIZE) ; #-4*TREE_SIZE (c0)
.buildHuffmanTree_calculateOffsets
	sta	nBitCode_literalOffset+4*TREE_SIZE-$100,x
	clc:adc	nBitCode_literalCount+4*TREE_SIZE-$100,x
	inx
...
.buildHuffmanTree_assignCode
	tya
	ldx	literalSymbolCodeLength,y
	ldy nBitCode_literalOffset,x:inc	nBitCode_literalOffset,x
	sta	codeToLiteralSymbol,y
	tay
	cpy	#CONTROL_SYMBOLS
	bcs	buildHuffmanTree_noControlSymbol2
	ldx	controlSymbolCodeLength,y
	ldy nBitCode_controlOffset,x:inc	nBitCode_controlOffset,x
...
; is it literal code of length X?
	sec:sbc	nBitCode_literalCount,x
...
.getNPlus1Bits_mask
	EQUB	GET_1_BIT,GET_2_BITS,GET_3_BITS,GET_4_BITS,GET_5_BITS,GET_6_BITS,GET_7_BITS

.inflateDynamic_tempSymbols
	EQUB	GET_2_BITS,GET_3_BITS,GET_7_BITS,0,8,7,9,6,10,5,11,4,12,3,13,2,14,1,15

.inflateDynamic_headerBits
	EQUB	GET_4_BITS,GET_5_BITS,GET_5_BITS
.inflateDynamic_headerBase
	EQUB	3,LENGTH_SYMBOLS,0
